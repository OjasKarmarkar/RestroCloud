...
function Yellow() {
  ...
  useEffect(() => {
    if (cachedMenu === "EMPTY MENU") {
      axios
        .get(`https://whispering-taiga-13938.herokuapp.com/api/menu`)
        .then((res) => {
        ...
        .catch(() => {
          console.log("backend might be down. Getting local menu");
          let menuWCat = categorize(menu);
          setMenuObj(menuWCat);
          dispatch(updateMenu(menuWCat));
          setLoading(false);
        });
  ...
  return (
    <motion.div
      ...
      exit={e}
      transition={t}
      className="page bg-slate-200 dark:bg-black dark:text-white pt-20">
      {loading && (
        ...
        <div className="px-24 py-8 flex flex-col md:flex-row justify-between h-screen">
          <div className="lg:mx-20 flex flex-col my-10 sm:my-0 gap-x-4 lg:gap-y-10 gap-y-8">
            <p className="text-neutral-500">
              (#Table ID : 628210ec1035265f44981a60)
            </p>
            <div className="lg:leading-normal text-3xl lg:text-5xl font-black">
              Eat. Treat. Repeat.
            </div>
            <div className="text-l font-normal lg:leading-normal">
              RestroCloud makes ordering food at your favorite restaurant
              simple. Say 'NO!' to remembering your desired order; and also to a
              goof-up spoiling your time!
            </div>
          ...
          <div className="flex-shrink-0 flex-grow-0 sm:m-10 sm:w-1/2">
